...
<template:addResources>
    <script type="text/javascript">
        $(document).ready(function () {
            $("#createForm_${currentNode.identifier}").submit(function(event) {
...
<form class="Form userInfoFormCreation" method="post"
      action="<c:url value='${url.base}${currentNode.path}/*'/>"
      id="createForm_${currentNode.identifier}">
    ...
    <div>
        <input id="createUserInfo${currentNode.name}" type="submit" class="button" value="OK" tabindex="23" />
    </div>
</form>

<ul>
<c:forEach items="${currentNode.nodes}" var="userInfo">
    <c:if test="${jcr:isNodeType(userInfo,'jnt:userInfo')}">
        <li><template:module node="${userInfo}" view="default"/></li>
    </c:if>
</c:forEach>
</ul>
